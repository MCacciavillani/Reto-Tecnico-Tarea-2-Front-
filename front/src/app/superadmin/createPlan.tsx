...
type Props = {
  setCreatePlan: React.Dispatch<React.SetStateAction<boolean>>;
  CP: boolean;
};

const CreatePlan: React.FC<Props> = ({setCreatePlan, CP}) => {
     ...
     const {
      ...
      const selectedType = watch("type");
        ...
        const onSubmit =async (data: FormData) => {
            try{
              const res= await axiosCustom.post("http://localhost:3001/plans", data)
        ...
        const onClick=()=>{
            ...
            setTimeout(() => setCreatePlan(false), 300);
      ...
      >
        {
            registerOK ? ( <div className="flex flex-col items-center justify-around h-full">
                ...
                <h1 className=" m-2 mt-10">Crear Plan</h1>
                <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col items-center mt-5">
                <input type="text" {...register("name")} placeholder="Nombre del plan" className="m-2 p-2 mt-5 border rounded"/>
                {errors.name && <p className="text-red-500 text-sm">{errors.name.message}</p>}
                <input type="text" {...register("description")} placeholder="Descripcion del plan" className="m-2 p-2 mt-5 border rounded"/>
                {errors.description && <p className="text-red-500 text-sm">{errors.description.message}</p>}
                <input type="number" step="0.01" {...register("price", { valueAsNumber: true })} placeholder="Precio del plan" className="m-2 p-2 mt-5 border rounded"/>
                {errors.price && <p className="text-red-500 text-sm">{errors.price.message}</p>}
                <input type="number" {...register("messageLimit", { valueAsNumber: true })} placeholder="Límite de mensajes" className="m-2 p-2 mt-5 border rounded"/>
                {errors.messageLimit && <p className="text-red-500 text-sm">{errors.messageLimit.message}</p>}
                <input type="text" {...register("type")} placeholder="Tipo de plan" className="m-2 p-2 mt-5 border rounded"/>
